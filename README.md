...
- `resume.html` — общий шаблон страницы резюме участника.
- `resume.js` — находит участника по параметру `member` в адресе и заполняет резюме.

При нажатии на имя открывается ссылка вроде `resume.html?member=ayaru`. Чтобы добавить нового участника, достаточно указать ему уникальный `slug` и заполнить объект `resume` в его файле.
